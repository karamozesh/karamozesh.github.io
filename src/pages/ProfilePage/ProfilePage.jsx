...
export default function ProfilePage() {
  ...
  <div>
    <div className='flex flex-row '>
      ...
      <br />
      <div onClick={toggleClass}  className='w-16 h-16 z-10  rounded-md bg-white  mr-1 shadow-[11px_20px_10px_-15px_rgba(0,0,0,0.3)] mt-5  '>
        <img  src={usericon} alt="" className='w-9 h-9 relative top-2 right-2 '/></div>
      <div onClick={toggleClass} className='hover:stroke-white w-16 h-16 rounded-md z-10 bg-white mr-1 shadow-[11px_20px_10px_-15px_rgba(0,0,0,0.3)] mt-3  '>
        <img  src={reqresult} alt="" className= ' w-9 h-9 relative top-2 right-2 '/></div>
      </div> 
      <div className={isActive ? 'hidden': null}><P1></P1></div>
      <div className={isActive ? null : 'hidden'}><P2></P2></div>
      </div>
  </div>) ;
}
