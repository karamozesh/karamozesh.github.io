             ...
             <h1 className="inline-block p-10 ">
              ...
              <section className=" bg-white flex sticky top-0 gap-5 justify-center mx-auto  mt-8 w-full   ">
               ...
               <section className='mx-auto  w-full flex justify-center'>
                ...
                <img src={roadmap} alt="" className='w-[80%] mr-8'/>
...
طبق نظرسنجی توسعه دهندگان StackOverflow در سال 2022، Statista، دانشگاه کالیفرنیا برکلی، و devjobsscanner.com،
جاوا اسکریپت پرتقاضاترین زبان برنامه نویسی در حال حاضر است.
<br />
2. جاوا اسکریپت ساده است.
جاوا اسکریپت یکی از ساده ترین زبان های برنامه نویسی با آموزش های آنلاین رایگان است. می توانید اصول اولیه را درک کنید و سپس HTML و CSS را اضافه کنید تا شروع به ساختن برنامه ها و وب سایت کنید. داشتن ساختار ساده یادگیری و پیاده‌سازی جاوا اسکریپت را آسان‌تر می‌کند و همچنین سریع‌تر از برخی از زبان‌های دیگر اجرا می‌شود. خطاها نیز به راحتی قابل تشخیص و تصحیح هستند.
<br />
3. سرعت
جاوا اسکریپت اسکریپت ها را مستقیماً در مرورگر وب بدون اتصال به سرور یا نیاز به کامپایلر اجرا می کند. علاوه بر این، اکثر مرورگرهای اصلی به جاوا اسکریپت اجازه می دهند تا کد را در طول اجرای برنامه کامپایل کند.
<br />
4. تطبیق پذیری
جاوا اسکریپت با زبان های دیگر مانند PHP، Perl و جاوا سازگار است. همچنین علم داده و یادگیری ماشین را برای توسعه دهندگان قابل دسترس می کند.
<br />
5. محبوبیت
منابع و انجمن های زیادی برای کمک به مبتدیان با مهارت های فنی محدود و دانش جاوا اسکریپت در دسترس هستند.
<br />
6. بار سرور
یکی دیگر از مزیت های عملکرد در سمت کلاینت این است که جاوا اسکریپت درخواست های ارسال شده به سرور را کاهش می دهد. اعتبار سنجی داده ها را می توان از طریق مرورگر وب انجام داد و به روز رسانی ها فقط برای بخش های خاصی از صفحه وب اعمال می شود.
<br />
7. به‌روزرسانی‌ها
تیم توسعه جاوا اسکریپت و ECMA International به‌طور مداوم چارچوب‌ها و کتابخانه‌های جدید را به‌روزرسانی و ایجاد می‌کنند و از ارتباط آن در صنعت اطمینان می‌دهند.
<br />
8 . توسعه دهندگان جاوا اسکریپت حقوق بالایی دارند.
در نظر بگیرید که میانگین حقوق یک توسعه دهنده جاوا اسکریپت در 20 سال گذشته تقریباً 35 درصد افزایش یافته است. علاوه بر این، افزایش دستمزد همچنان در حال افزایش سالانه است، به طوری که میانگین توسعه‌دهنده جاوا اسکریپت در حال حاضر حدود 97577 دلار تا 119839 دلار در سال درآمد دارد.
<br />
9 . جاوا اسکریپت را می توان در همه جا استفاده کرد
فقط یک زبان را یاد بگیرید و از آن در همه جا استفاده کنید.
جاوا اسکریپت را می توان برای کدهای front-end و back-end (Full Stack) استفاده کرد.
هنگامی که اصول جاوا اسکریپت را به پایان رساندید، می توانید React، Next، Nust، React Native، Node.js، Express.js و موارد دیگر را بیاموزید.
می تواند درفرانت اند استفاده شود: React Js و Next Js به عنوان معروف ترین فریمورک زبان جاوا اسکریپت، کل دنیای توسعه برنامه های وب را پوشش داده اند. برخی از برترین محصولات تعیین کننده بازار (اینستاگرام، ردیت و فیس بوک واضح ترین نمونه ها هستند).
می تواند در برنامه های تلفن همراه Frontend استفاده شود:: React Native بهترین بخش های توسعه بومی را با React ترکیب می کند، بهترین کتابخانه جاوا اسکریپت در کلاس برای ساخت رابط های کاربری. ما می‌توانیم با استفاده از جاوا اسکریپت، برنامه‌های بومی برای Android، iOS و موارد دیگر ایجاد کنیم
می تواند در Backend استفاده شود: در همین حال، توسعه بک اند روی سمت وب سایتی تمرکز می کند که کاربران نمی توانند ببینند (سمت سرور). به عنوان مثال مدیریت پایگاه داده و پاسخ به تماس های frontend.
متداول‌ترین فریم ورک‌هایی که امروزه استفاده می‌شوند Node JS، Express Js، NUST js و GraphQL برای توسعه Backend هستند.
با یادگیری تنها یک زبان جاوا اسکریپت (React + Node) شما به یک "توسعه‌دهنده فول استک" تبدیل می‌شوید.
با رشد روزافزون اینترنت اشیاء، نیاز به برنامه‌های کاربردی و پویا در این زمینه افزایش یافته است. جاوا اسکریپت به عنوان یک زبان برنامه‌نویسی چندمنظوره، در توسعه برنامه‌های کاربردی برای اینترنت اشیاء نیز مورد استفاده قرار می‌گیرد.
با توجه به اینکه اینترنت اشیاء در آینده رشد قابل توجهی خواهد داشت، یادگیری جاوا اسکریپت به عنوان یکی از زبان‌های برنامه‌نویسی اصلی در این حوزه، برای توسعه دهندگان بسیار مفید و ضروری است.
<br />
...
2. امنیت
کد جاوا اسکریپت که در سمت کلاینت اجرا می شود در برابر سوء استفاده توسط کاربران غیرمسئول آسیب پذیر است. <br />
3. اشکال زدایی
در حالی که برخی از ویرایشگرهای HTML از اشکال زدایی پشتیبانی می کنند، نسبت به سایر ویرایشگرها کارایی کمتری دارند. از آنجایی که مرورگرها <br /> هیچ هشداری برای خطا نشان نمی دهند، پیدا کردن مشکل ممکن است چالش برانگیز باشد.</p>
